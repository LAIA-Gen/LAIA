...
def CRUDShardController(repository: ModelRepository, jwtSecretKey: str='secret_key', auth_required: bool = False) -> APIRouter:
    ...
    @router.post("/shard/", response_model=Shard)
    ...
    @router.put("/shard/{element_id}", response_model=Shard)
    ...
    @router.get("/shard/{element_id}", response_model=Shard)
    ...
    @router.post("/shards/", response_model=SearchResponse)
    async def search_shards(token: get_auth_dependency() = None, skip: int = 0, limit: int = 10, filters: dict = {}, orders: dict = {}, populate: Optional[List[str]] = None):
        user_roles = await get_user_roles(repository, token, jwtSecretKey)
